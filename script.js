...
Display the first and last name of the sales agent. Object.values
Display all keys and values for the car sold. Object.entries
Display the gross profit made on the sale. Object.values
...
// Display all keys and values for the car sold. Object.entries
// Display the gross profit made on the sale. Object.values
const displayOnDOM = document.querySelector("#salesReport")
const displayListOnDOM = document.querySelector("#keysAndValues")
let firstName = ""
let lastName = ""
let profit = ""
displayOnDOM.innerHTML = `<h1>Sales Report</h1><hr>`
salesByWeek.forEach(sale => {
    firstName = sale.sales_agent.first_name
    lastName = sale.sales_agent.last_name
    profit = sale.gross_profit
    displayOnDOM.innerHTML += `<h2>${firstName} ${lastName}</h2>`

    for (const keysAndValues of Object.entries(sale.vehicle)) {
        console.log(keysAndValues)
        displayOnDOM.innerHTML += `
        <div>${keysAndValues[0]}: ${keysAndValues[1]}</div>
        `
    }
    displayOnDOM.innerHTML += `<br><strong>Profit: $${profit}</strong></p><hr>`
})